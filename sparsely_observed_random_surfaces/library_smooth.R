...
CV_s <- function(X,Folds=10,WN_flag=T,maxiter=5,miniter=1,wopt_flag=F){
  N <- dim(X)[1]
  K1 <- dim(X)[2]
  K2 <- dim(X)[3]
  B <- array(1, c(K2,K2))
  ...
  Fit <- array(0,c(Folds,maxiter-miniter+1))
  Norms <- rep(0,maxiter-miniter+1)
  
  Res <- estimate_sparse_cv(X,WN_flag,maxiter,wopt_flag)
  for(i in 1:maxiter){
    Norms[i] <- frobenius(Res$A[i,,])*frobenius(Res$B[i,,])
  }
  for(fold in 1:Folds){
    ...
    Xtest <- X[Ind[fold,],,]
    Ntest <- dim(Xtest)[1]
    Res <- estimate_sparse_cv(Xtrain,WN_flag,maxiter,wopt_flag)
    for(iter in miniter:maxiter){
      akt_fit <- rep(0,Ntest)
      norm_C_n_obs <- rep(0,Ntest)
      for(n in 1:Ntest){
        X_akt <- Xtest[n,,]
        X_akt[is.na(X_akt)] <- 0
        X_outer <- outer(X_akt,X_akt)
        X_outer <- zero_diag(X_outer)
        C_n <- aperm(outer(Res$A[iter,,],Res$B[iter,,]),c(1,3,2,4))
        # sparse-inner-product evaluation with numerator weight (denominator gets canceled)
        akt_fit[n] <- sum(C_n*X_outer)*frobenius(Res$A[iter,,])^2*frobenius(Res$B[iter,,])^2
        # norm calculation for calculating the weighted average below (only denominator, numerator gets canceled with the above)
        obsind <- !is.na(Xtest[n,,])
        obsind <- outer(obsind,obsind)
        obsind <- zero_diag(obsind)
        norm_C_n_obs[n] <- frobenius(aperm(outer(Res$A[iter,,],Res$B[iter,,]),c(1,3,2,4))*obsind)^2
      }
      Fit[fold,iter] <- sum(akt_fit)/sum(norm_C_n_obs)
    }
  }
  Fit <- colMeans(Fit) # averaging over Folds
  return(Norms^2-2*Fit)
}

zero_diag <- function(X){
  d <- dim(X)[1:2]
  Xmat <- tensor2matrix(X)
  diag(Xmat) <- 0
  X <- matrix2tensor(Xmat,d[1],d[2])
  return(X)
}

estimate_sparse_cv <- function(X,WN_flag,maxiter,wopt_flag,B=NULL,bwidths=c(NULL,NULL),tol=1e-6,sparse=F){
  N <- dim(X)[1]
  K1 <- dim(X)[2]
  K2 <- dim(X)[3]
  if(length(B) == 0){
  ...
  As <- array(0,c(maxiter,K1,K1))
  Bs <- array(0,c(maxiter,K2,K2))
  eps <- 1
  iter <- 1
  while(iter <= maxiter && eps > tol){
    A <- T1smooth(X,B,N,K1,K2,bwidths[1],WN_flag,wopt_flag) # estimate temporal covariance given the spatial covariance
    As[iter,,] <- A <- A$A/frobenius(A$A)                       # standardize to prevent mass escape
    B <- T2smooth(X,A,N,K1,K2,bwidths[2],WN_flag,wopt_flag) # estimate spatial covariance given the temporal covariance
    Bs[iter,,] <- B <- B$B
    iter <- iter +1
...
estimate_sparse <- function(X,WN_flag=F,maxiter=2,B=NULL,bwidths=c(NULL,NULL),tol=1e-6,sparse=F,wopt_flag=F){
...
#   WN_flag - if true, a white noise errors are assumed and diagonals are discarded before every smoothing step
# wopt_flag - if true, the optimal weights of Zhang and Wang (2018) are used
...
estimate_sparse_sim <- function(X,WN_flag=F,Cov1,Cov2,maxiter=2,B=NULL,bwidths=c(NULL,NULL),tol=1e-10,sparse=F,wopt_flag=F){
...
estimate_sparse_gneiting <- function(X,WN_flag=F,C,maxiter=2,B=NULL,bwidths=c(NULL,NULL),tol=1e-6,sparse=F,wopt_flag=F){
...
T1smooth <- function(X,B,N,K1,K2,bandwidth,WN_flag=F,wopt_flag=F){
# partial inner product w.r.t. to the first argument and subsequent smoothing, i.e. returning A (temporal)
  ...
  Ns <- rep(0,N)# N_i for calculating the optimal weight by Zhang and Wang (2018)
  if(WN_flag) diag(B) <- 0
  ...
  for(n in 1:N){
    ...
    Res[n,,] <- Res_akt/W
    Win[n,,] <- W
    Ns[n] <- sum(!is.na(Res_akt)) # for optimal weights calculation by Zhang and Wang (2018)
    # Ns[n] <- sum(!is.na(X_akt)) # for optimal weights calculation by Zhang and Wang (2018)
  }
  ...
  # recalculate the weights using the optimal ones per surface by Zhang and Wang (2018)
  if(wopt_flag){
    v_i <- 0
    vs <- rep(0,N)
    denom <- 0
    for(n in 1:N){
      W <- Win[n,,]
      diag(W) <- 0
      b1 <- calc_b1(X[n,,],abs(B))
      b2 <- calc_b2(X[n,,],abs(B))
      b3 <- calc_b3(X[n,,],abs(B))
      b4 <- calc_b4_all(X[n,,],abs(B)) - b2 - b3
      vs[n] <- b1*( bandwidth^{-2}*b2 + bandwidth^{-1}*b3 + b4 )^{-1}
      denom <- denom + b1*vs[n]
    }
    for(n in 1:N){
      Win[n,,] <- sqrt(vs[n])*Win[n,,]#/denom
    }
    Weights <- melt(Win[1,,])
    Weights <- Weights[complete.cases(Weights),]
    for(n in 2:N){
...
calc_b1 <- function(X,B){ # send X as t(X) if A instead of B
  Ind <- !is.na(X)
  ind <- colSums(Ind)
  # ind <- as.numeric(I(ind> 0))
  Ind <- outer(ind,ind)
  diag(Ind) <- 0
  return(sum(Ind*B))
}

calc_b2 <- function(X,B){ # send X as t(X) if A instead of B
  Ind <- !is.na(X)
  ind <- colSums(Ind)
  # ind <- as.numeric(I(ind> 0))
  Ind <- outer(ind,ind)
  diag(Ind) <- 0
  return(sum(Ind^2*B^2))
}

calc_b3 <- function(X,B){
  K <- dim(X)[2]
  Ind <- !is.na(X)
  ind <- colSums(Ind)
  # ind <- as.numeric(I(ind> 0))
  ...
  for(i in 1:K){
    for(j in 1:K){
      for(k in 1:K){
        Res <- Res + Ind[i,j]*Ind[i,k]*B[i,j]*B[i,k]
      }
      Res <- Res - Ind[i,j]^2*B[i,j]^2
      for(k in 1:K){
        Res <- Res + Ind[i,j]*Ind[k,j]*B[i,j]*B[k,j]
      }
      Res <- Res - Ind[i,j]^2*B[i,j]^2
    }
  }
...
T2smooth <- function(X,A,N,K1,K2,bandwidth,WN_flag=F,wopt_flag=F){
# partial inner product w.r.t. to the second argument and subsequent smoothing, i.e. returning B (spatial)
  ...
  if(wopt_flag){
    ...
    for(n in 1:N){
      W <- Win[n,,]
      diag(W) <- 0
      b1 <- calc_b1(t(X[n,,]),abs(A))
      b2 <- calc_b2(t(X[n,,]),abs(A))
      b3 <- calc_b3(t(X[n,,]),abs(A))
      b4 <- calc_b4_all(t(X[n,,]),abs(A)) - b2 - b3
      vs[n] <- b1*( bandwidth^{-2}*b2 + bandwidth^{-1}*b3 + b4 )^{-1}
      denom <- denom + b1*vs[n]
    }
    for(n in 1:N){
...
# Output: data set Y in the format specified by 'sparse'
  ...
  # subsample data
  for(n in 1:N){
    if(vary) n_points <- sample(3:(5*floor(K1*K2*perc/100)),1)
...
run_sim <- function(N,K1,K2,perc,maxiter=20,seed=517,covtype="brownian",WN_flag=F,vary=F,sparse=F,wopt_flag=F){
  # simulate data
  set.seed(seed)
...
# T1smooth(X,B,N,K1,K2,F)
  Res <- estimate_sparse(X,maxiter,B=B,WN_flag=WN_flag,sparse=F,wopt_flag)
