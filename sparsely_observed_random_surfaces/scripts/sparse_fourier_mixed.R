...
simulate <- function(sim){
  ...
  for(pp in 1:7){ 
    ...
    print(perc)
    set.seed(17*sim)
    X <- simulate_data(N,perc,A,B,WN_flag=T,vary=F)
    ...
    Res4 <- estimate_sparse_sim(X,WN_flag=T,maxiter=5,Cov1=A,Cov2=B,wopt_flag=T)
    source("library_smooth5.R")
    Res5 <- estimate_sparse_sim(X,WN_flag=T,maxiter=5,Cov1=A,Cov2=B,wopt_flag=T)
...
save(Data, file="sparse_fourier_data_add.RData")
