...
simulate <- function(sim){
  ...
  for(pp in 1:7){ 
    ...
    for(n in 1:N){
      n_points <- sample(3:(5*floor(K1*K2*perc/100)),1)
    ...
    source("library_smooth.R")
    Res1 <- estimate_sparse_gneiting(X,WN_flag=T,maxiter=5,C=C,wopt_flag=F)
    # ERR[1:3,pp] <- Res$err
    # cv1 <- CV_s(X,wopt_flag=F)
    source("library_smooth.R")
    Res2 <- estimate_sparse_gneiting(X,WN_flag=T,maxiter=5,C=C,wopt_flag=T)
    # ERR[1:3,pp] <- Res$err
    # cv2 <- CV_s(X,wopt_flag=T)
    source("library_smooth2.R")
    Res3 <- estimate_sparse_gneiting(X,WN_flag=T,maxiter=5,C=C,wopt_flag=T)
    # ERR[1:3,pp] <- Res$err
    # cv3 <- CV_s(X,wopt_flag=T)
    source("library_smooth4.R")
    Res4 <- estimate_sparse_gneiting(X,WN_flag=T,maxiter=5,C=C,wopt_flag=T)
    source("library_smooth5.R")
    Res5 <- estimate_sparse_gneiting(X,WN_flag=T,maxiter=5,C=C,wopt_flag=T)
    
    ( Res <- t(matrix(c(Res1$err,Res2$err,Res3$err,Res4$err,Res5$err),ncol=5)) )
