...
simulate <- function(sim){
  RES <- array(0,c(11,7))
    delta <- 0
    # Dat <- input[[sim]]
    Dat <- create_data(seed=17*sim,buf=0,A1="brownian",A2="brownian",mask="tiangular",tau=0.5,snr=3)
  ...
  for(buf in 0:9){
    print(buf)
    delta <- 2*buf+1
    Dat <- create_data(seed=17*sim,buf=buf,A1="brownian",A2="brownian",mask="triangular",tau=0.5,snr=3)
...
save(data, file="brownian_tri_buf.RData")
