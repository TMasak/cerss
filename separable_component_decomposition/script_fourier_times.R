source("library_PIP.R")
  ...
  Ks <- seq(50,350,by=30)
  ERR <- array(0,c(length(Ks),30)) # sample size x [R=1,2,3,4 + empirical + 
...
for(kk in 1:length(Ks)){
  ...
  As[1,,] <- V[,order1] %*% diag(c(rate_in^(50:1),rep(0,K-50))) %*% t(V[,order1])
  As[2,,] <- V[,order2] %*% diag(c(rate_in^(50:1),rep(0,K-50))) %*% t(V[,order2])
  As[3,,] <- V[,order3] %*% diag(c(rate_in^(50:1),rep(0,K-50))) %*% t(V[,order3])
  As[1,,] <- As[1,,]/frobenius(As[1,,])
  As[2,,] <- As[2,,]/frobenius(As[2,,])
  As[3,,] <- As[3,,]/frobenius(As[3,,])
  As[1,,] <- As[1,,] + array(0.1,c(K,K))/K
  As[2,,] <- As[2,,] + array(0.1,c(K,K))/K
  As[3,,] <- As[3,,] + array(0.1,c(K,K))/K
  As[1,,] <- As[1,,]/frobenius(As[1,,])
  As[2,,] <- As[2,,]/frobenius(As[2,,])
  As[3,,] <- As[3,,]/frobenius(As[3,,])
  ...
  for(r in 1:R){
    ...
    N <- 200
    Ntest <- 25
    akt_err <- array(0,c(10,Ntest)) # [R=1,2,3,4 + empirical] x [test samples]
    akt_times <- array(0,c(5,Ntest))
    akt_iter <- array(0,c(5,Ntest))
    set.seed(517*4)
    X <- array(0,c(N,K,K))
    Xtest <- array(0,c(Ntest,K1,K2))
    for(n in 1:N){
      for(r in 1:R){
    ...
    ERR[kk,11] <- difftime(Sys.time(),t,units="secs")
    t <- Sys.time()
    Res <- separable_expansion(X,2,10)
    ERR[kk,12] <- difftime(Sys.time(),t,units="secs")
    t <- Sys.time()
    Res <- separable_expansion(X,3,10)
    ERR[kk,13] <- difftime(Sys.time(),t,units="secs")
    # ERR[nn,13] <- CV2(X,8,3)
    ...
    for(r in 1:R){
      ...
      for(n in 1:Ntest){
        ...
        akt_err[r,n] <- frobenius(Xtest[n,,] - Xhat$Xhat)/frobenius(Xtest[n,,])
        akt_err[r+5,n] <- frobenius(Xtest[n,,] - Xhat$Xhat)/frobenius(Xtest[n,,]*(!Ind))
        akt_iter[r,n] <- Xhat$iter
      }
    }
  ...
  if(K < 150){ 
    ...
    C_hat <- C_hat/N
    ERR[kk,15] <- difftime(Sys.time(),t,units="secs")
    ...
    ERR[kk,16:25] <- rowMeans(akt_err)
    ERR[kk,6:10] <- rowMeans(akt_times)
    ERR[kk,26:30] <- rowMeans(akt_iter)
}

save(ERR,file="data_fourier_times.RData")
